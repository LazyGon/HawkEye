...
public class MonitorEntityListener extends HawkEyeListener {
	...
	/**
	 * Uses the lastAttacker field in the players {@link PlayerSession} to log the
	 * death and cause
	 */
		...
		if (entity instanceof Player) { // Player death
			...
			if (victim.getLastDamageCause() instanceof EntityDamageByEntityEvent) {
				...
				if (damager instanceof Player) {
					if (!Config.isLogged(DataType.PVP_DEATH) && !Config.LogDeathDrops)
						return;
					DataManager.addEntry(new DataEntry(victim, DataType.PVP_DEATH, victim.getLocation(),
							Util.getEntityName(damager)));
				} else {
					if (!Config.isLogged(DataType.MOB_DEATH) && !Config.LogDeathDrops)
						return;
					DataManager.addEntry(new DataEntry(victim, DataType.MOB_DEATH, victim.getLocation(),
							Util.getEntityName(damager)));
				}
				// Other death
			} else {
			...
			if (entity.getLastDamageCause() instanceof EntityDamageByEntityEvent) {
				...
				DataManager.addEntry(
						new DataEntry(player, DataType.ENTITY_KILL, entity.getLocation(), Util.getEntityName(entity)));
			}
		}
	}
	...
	public void onPaintingBreak(PaintingBreakEvent event) {
		...
		if (e.getRemover() instanceof Player)
			DataManager.addEntry(
					new DataEntry((Player) e.getRemover(), DataType.PAINTING_BREAK, e.getPainting().getLocation(), ""));
	}

	@HawkEvent(dataType = DataType.PAINTING_PLACE)
	public void onPaintingPlace(PaintingPlaceEvent event) {
		DataManager.addEntry(
				new DataEntry(event.getPlayer(), DataType.PAINTING_PLACE, event.getPainting().getLocation(), ""));
	}
	...
	public void onEntityChangeBlock(EntityChangeBlockEvent event) {
		...
		if (event.getTo() == Material.AIR && Config.isLogged(DataType.ENDERMAN_PICKUP)) {
			...
			DataManager.addEntry(new BlockChangeEntry("Environment", DataType.ENDERMAN_PLACE, block.getLocation(),
