...
public class InventoryUtil {
	...
	/**
	 * Compress an ItemStack[] into a HashMap of the item string and the total
	 * amount of that item Uses {@BlockUtil} to get the item string
	 * 
	...
	/**
	 * Takes two compressed inventories and returns a string representation of the
	 * difference
	 * 
	 ...
	 *         item:data,amount&item:data,amount@item:data,amount&item:data,amount
	...
	/**
	 * Takes an inventory difference string and forms two HashMaps containing the
	 * compressed inventory forms of the additions and subtractions
	 * 
	 ...
	 * 
	 * @param ops additions and subtractions as supplied by
	...
	public static String createChangeString(List<HashMap<String, Integer>> ops) {
		...
		// Build string
		if (add.size() > 0)
			changeString += "&a+(" + Util.join(add, ", ") + ")";
		if (sub.size() > 0)
			changeString += "&4-(" + Util.join(sub, ", ") + ")";
	...
	/**
	 * Method for getting complete inventory from a ContainerBlock Works around a
	 * bug in Minecraft that sometimes returns only half the chest Thanks to N3X15
	 * and the BigBrother team for letting me use this
	 * 
